...
**Update an existing doc:** edit the Google Doc. The scheduled sync publishes
the page, and uploads a changed PDF to R2, without a local sync. Re-run
the sync locally only to preview before that run.
...
- **Conversation** — a sibling Worker in `agent/` (own `wrangler.jsonc`,
  **not** part of the Pages project). `ChatAgent` is a Durable Object that
  holds one conversation per browser session and calls the index through a
  `searchDocs` tool. The live answer path is `CHAT_MODE=toolloop`: GPT-5.6
  Luna, reached through AI Gateway, writes from the retrieved chunks only.
  Rate-limited per conversation; CORS is an origin allowlist.
- **Widget** — vanilla JS on the docs site. Source is
  ...
  `searchDocs` tool output** — never from the model's prose. The widget
  talks to `localhost:5173` on a local preview and to
  `nanodocs-agent.nanofab.workers.dev` on `nanodocs.pages.dev`.
...
The Worker is live. A push to `main` does not update it — see
[Working on the repo](#working-on-the-repo). Reader-facing explanation:
[How the Docs Chat Works](docs/authoring/how_chat_works.md). Technical
walkthrough: [Wiring a chat agent](docs/authoring/chat_agent.md).

## Working on the repo

The site and the chat agent are two Cloudflare products. Updating one
does not update the other.

```text
git push origin main
    → Cloudflare Pages rebuilds the site and the chat button
    → https://nanodocs.pages.dev is new
    → the button still calls the previously deployed agent

cd agent && npm run deploy
    → uploads the nanodocs-agent Worker
    → that is the prompt, the model, CHAT_MODE, and the search binding
```

### Catch up first

A clone that has been sitting still falls behind the scheduled doc sync.
Those commits are all titled `Sync Google Docs`.

```bash
git pull
```

If Git stops and tells you to set `pull.rebase`, do not change git
config. The branches diverged because this clone has a local commit and
GitHub has new sync commits. Reconcile once with:

```bash
git pull --rebase
```

Pulling updates your copy of the SOP pages. It does not deploy the agent.

The schedule is `.github/workflows/sync-and-publish.yml` (several times
an hour, off the quarter-hour). When a Google Doc's export hash changed,
the workflow syncs, builds the site strictly, uploads any changed PDF to
R2, commits, and pushes `main`. That push is what republishes the site.
A run that finds nothing changed does not commit.

### Change the site

Pages, `mkdocs.yml`, styles, or the chat button's look. Do not hand-edit
a page that starts with `<!-- AUTO-GENERATED by scripts/sync_gdocs.py`.
Edit the Google Doc and let the scheduled sync publish it. To preview a
doc on this machine first, run the sync (see above) and
`uv run zensical serve`.

Before pushing a hand edit:
...
If you edited `agent/widget/chat-widget.js`, rebuild the bundle from
`agent/` with `npm run build:widget` and commit
`overrides/javascripts/chat-widget.js` with the push. That updates the
button in the browser. The agent that writes the answers is unchanged.

### Change the agent

`agent/src/server.ts`, `agent/wrangler.jsonc`, the model, or `CHAT_MODE`.

```bash
cd agent
npm run dev       # http://localhost:5173/ — try it in a new chat
npm run deploy    # this is what updates the live agent
```

Always `cd agent` before Wrangler. From the repo root it uses the Pages
`wrangler.jsonc` instead. Commit and push the source so GitHub has it;
that push does not update the running Worker. After `npm run deploy`,
test on the live site in a new chat. An existing thread keeps the replies
it already stored.

### Both

Push `main` for the site, and run `npm run deploy` from `agent/` for the
agent.
...
On `main`, the scheduled workflow does this. Run it yourself when you
need the live file updated before the next poll, or when you are
previewing locally:

1. Sync (`uv run python scripts/sync_gdocs.py …`). That refreshes the
   local file under `docs/assets/pdfs/{tools,chem,policy}/`.
